...
Position next_coordinates(double init_x, double init_y, double arm_length, double current_orientation, double new_angle) {
    ...
    //std::cout << "x: " << pos.x << ", z: " << pos.z << ", orient: " << pos.orient << "\n";
    return pos;
}

std::vector<JointState> generate_trajectory(JointState start, JointState target, double time_step) {
    
    double max_time = 0.0;
    std::vector<JointState> trajectory;
    
    std::vector<double> changes_in_angles = { target.U1 - start.U1, target.U2 - start.U2, target.R1 - start.R1, target.R2 - start.R2, target.P1 - start.P1 };
    std::vector<double> speeds = {30, 30, 45, 45, 10};
    std::vector<double> acceleration = {120, 120, 180, 180, 50};

    for (size_t i = 0; i < changes_in_angles.size(); i++) {
        double changes = std::abs(changes_in_angles[i]);
...
int main() {
    std::vector<std::pair<JointState, JointState>> testCases = {
    ...
    for (std::pair<JointState, JointState> testCase : testCases) {
        JointState start = testCase.first;
        JointState target = testCase.second;
        ...
        std::cout << "trajectory\n";
        std::vector<JointState> trajectory = generate_trajectory(start, target, 1.0 / 150.0);
        for (JointState state : trajectory) {
            std::cout << "(" << state.U1 << ", " << state.U2 << ", " << state.R1 << ", " << state.R2 << ", " << state.P1 << ")\n";
        }
    }
